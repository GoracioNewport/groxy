...
[Timer]
# Три часа ночи по местному времени, а не полночь. Обновление перезапускает
# dnsmasq, а с Фазы 3 будет перезапускать и Xray — а тот рвёт все
# проксируемые соединения клиентов. В полночь людей в сети заметно больше.
# Разведено с ежедневным бэкапом в 03:40, чтобы не толкаться за общую
# блокировку.
OnCalendar=*-*-* 03:00:00
