...
В репозитории лежит [Claude Code](https://claude.com/claude-code)-скилл для интерактивного управления groxy-инсталляцией — установка, добавление/удаление клиентов, переключение между portal'ами, диагностика. Если ты используешь Claude Code, можно подключить его одной командой:

```sh
ln -s "$(pwd)/.claude/skills/groxy" ~/.claude/skills/groxy
```

После этого скажи Claude'у что-то вроде «*установи groxy на мой sweden-vps*» или «*добавь клиента phone с QR*» или «*почему handshake не сходится*» — скилл активируется и проведёт через нужный workflow с SSH-доступом, без необходимости держать в голове команды и пути файлов.

Файл скилла — [`.claude/skills/groxy/SKILL.md`](.claude/skills/groxy/SKILL.md). Можно прочитать руками и без Claude'a — это структурированный гайд по всем основным операциям и диагностике.
